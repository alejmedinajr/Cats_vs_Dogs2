...
Finally, we need to make all of the states lower cases so it can be joined with the state map data set.
```{r}
...
state_map_pets <- left_join(state_map,pet_data, by = "region")
head(state_map_pets)
fourK_graph <- ggplot(state_map_pets, aes(x= long, y = lat, group = group)) +
  geom_polygon(aes(fill = spend4kPerc)) +
  scale_fill_viridis_c() # Create map on total house percentage
fourK_graph # View graph

```
<br><b>Most states were under 45% in regards to whether they would spend up to $4k to save their dog. The states with higher percentages (45%-50%) were located along the mid West coast and the upper East coast.</b>
...
Now that the data has been cleaned, we can use it to create a map and compare it to. First, here is the map seeing the percentage of people that would spend 4K to save their dog.
```{r}
...
Next we'll plot the median income per state
